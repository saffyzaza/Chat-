...
const response = await fetch('/api/admin/mental', {
method: 'POST',
headers: {
...
<div className="min-h-screen bg-[#f8f9fa]">
      ...
      <div className="bg-white border-b border-gray-200 h-16 flex items-center px-4 sm:px-6 lg:px-8 shrink-0">
        <Link 
          href="/admin" 
          className="mr-4 p-2 text-gray-500 hover:text-pink-600 hover:bg-pink-50 rounded-full transition-colors"
        >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-800">ช่องแชทสุขภาพจิต (Mental Health Chat)</h1>
            <p className="text-xs text-gray-500">ระบบให้คำปรึกษาและติดตามผู้ป่วยด้านสุขภาพจิตด้วย AI</p>
          </div>
        </div>
      </div>
...
ระบบจะตอบคำถามเกี่ยวกับสถิติ ตัวเลข เป้าหมาย และผลการดำเนินงานด้านสุขภาพจิตของหน่วยงานต่างๆ ตามฐานข้อมูลที่มี
</div>
...
<p className="text-sm">เริ่มต้นแชทได้เลย เช่น "ขอสรุปผลงานสุขภาพจิตของโรงพยาบาล..." หรือ "เป้าหมายเทียบกับผลงานปีนี้เป็นอย่างไร"</p>
</div>
) : (
...
message.role === 'user'
? 'bg-white border-pink-100 ml-auto'
: 'bg-blue-50 border-blue-100 mr-auto'
}`}
>
<div className="text-xs text-gray-500 mb-1 flex items-center gap-1">
                    <span className={`w-2 h-2 rounded-full ${message.role === 'user' ? 'bg-pink-400' : 'bg-blue-400'}`}></span>
...
disabled={loading}
className="flex-1 min-h-[50px] max-h-[150px] border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 transition-all resize-none"
              rows={1}
/>
<button
onClick={sendMessage}
disabled={loading || !input.trim()}
className="px-6 py-2 bg-pink-600 hover:bg-pink-700 text-white font-medium rounded-lg disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed transition-colors self-end h-[46px]"
>
ส่ง
</button>
</div>
